...
//     const ctx = this._getContext(transactionId);
//     const isFinalItem = ctx.currentItemIndex === ctx.items.length - 1;
//
...
//
//       return {
//         success: response?.isSuccess && Object.values(response.lockStatuses || {}).every((l) => l === LOCK_STATUS.OPENED),
//       };
//     } catch (error) {
...
//
//     await this._calculationService.onLockOpenSuccess({
//       transactionId: ctx.transactionId,
//       data: item,
...
//       updatedTransaction.locations = updatedTransaction.locationsTemp;
...
//     this._logger.log(`Handling issue items for transaction ${transaction.id}, bin ${binId}`);
//     const itemTypeReplenish = await this._repository.findReplenishItemType();
//     for (const location of transaction.locations) {
//       if (location.bin.id === binId) {
//         for (const item of location.spares) {
//           const actualQty = Math.abs(item.changedQty);
//           if (!itemTypeReplenish.includes(item.type) && actualQty != 0) {
//             const returnItem = await this._itemRepository.findReturnItem({
//               userId,
//               binId,
//               itemId: item.id,
//             });
//             if (returnItem) {
//               let locations = [];
//               const isLocationExisted = returnItem.locations.find((item) => {
//                 return item.bin.id === binId;
//               });
//
//               if (isLocationExisted) {
//                 locations = returnItem.locations.map((item) => {
//                   if (item.bin.id === binId) {
...
//                         cuId: location.bin.cuId,
//                         lockId: location.bin.lockId,
//                       },
//                       quantity: actualQty,
//                     },
//                   ],
//                 ];
//               }
//               if (returnItem.workingOrders && item.workingOrders) {
//                 returnItem.workingOrders = this._mergeWorkingOrders([...returnItem.workingOrders, ...item.workingOrders]) as any;
//               }
//               returnItem.locations = locations;
//               returnItem.quantity += actualQty;
//               await this._itemRepository.updateReturnItem(returnItem.id, returnItem);
//             } else {
//               const locations = [
//                 {
//                   cabinet: {
...
//               ];
//
//               await this._itemRepository.createReturnItem({
//                 userId: userId,
//                 itemId: item.id,
//                 binId: binId,
...
//     this._logger.log(`Handling return items for transaction ${transaction.id}, bin ${binId}`);
//     for (const location of transaction.locations) {
//       this._logger.warn('location', location);
//       if (location.bin.id === binId) {
//         for (const item of location.spares) {
//           this._logger.warn('item', item);
//           const actualQty = item.changedQty;
//           const returnItem = await this._itemRepository.findReturnItem({
//             userId,
//             itemId: item.id,
//             binId,
//           });
//           this._logger.warn('returnItem', { returnItem });
//           if (returnItem) {
//             returnItem.locations = returnItem.locations.map((item) => {
//               if (item.bin.id === binId) {
//                 item.quantity -= actualQty;
//               }
//               return item;
...
//             returnItem.quantity -= actualQty;
//             await this._itemRepository.updateReturnItem(returnItem.id, returnItem);
//             this._logger.warn('returnItem.quantity', { quantity: returnItem.quantity });
...
//     await this._publisherService.publish(Transport.MQTT, 'processItem/success', {});
//   }
//
