...
const CALCULATION_CONFIG = {
  ...
  CONDITION_WORKING_ID: process.env.CONDITION_WORKING_ID || 'normal',
};
...
export class CalculationService {
  ...
  public async handleLockOpenSuccess(message: LockOpenSuccessMessage): Promise<ProcessResult> {
    ...
    try {
      ...
      await this._publishProcessingStatus(true, false);
      ...
      await this._publishProcessingStatus(result.isProcessingItem, result.isNextRequestItem);
      ...
      await this._publishProcessingStatus(errorResult.isProcessingItem, errorResult.isNextRequestItem);
  ...
  private async _waitForLockClosure(message: LockOpenSuccessMessage): Promise<void> {
    ...
    while (new Date().getTime() - startTime.getTime() < timeout) {
      ...
      try {
        const response = await this._publisherService.publish<CuResponse>(Transport.TCP, '', lockStatusPayload);
    ...
    if (newDevices.length > 0 && currentDevices.length > 0) {
      for (const newDevice of newDevices) {
        ...
        const processItemLog: ProcessItemLog = {
          ...
          type: item.type,
          conditionName: CALCULATION_CONFIG.CONDITION_WORKING_ID || 'normal',
          quantity: actualQty,
  ...
  private async _publishProcessingStatus(isProcessingItem: boolean, isNextRequestItem: boolean): Promise<void> {
    try {
      const statusData = { isProcessingItem, isNextRequestItem };
  ...
  /**
   * The Main entry point for MQTT message 'lock/openSuccess'
   * Immediately emits processing status and handles calculation
   */
  public async onLockOpenSuccess(payload: LockOpenSuccessMessage): Promise<ProcessResult> {
    try {
      this._logger.log('Received lock open success event', { timestamp: new Date() });

      // Handle the calculation process and return result
