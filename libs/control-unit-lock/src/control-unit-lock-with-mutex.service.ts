...
class SerialTimeoutError extends Error {
...
@Injectable()
export class ControlUnitLockWithMutexService implements OnModuleInit {
  private readonly _logger: Logger = new Logger(ControlUnitLockWithMutexService.name);
  private readonly _controlUnitLockPorts: string[] = [];
  private readonly _subControlUnitPorts: string[] = [];

  private readonly _portMutexes = new Map<string, Mutex>();
  ...
  private _getMutexForPort(port: string): Mutex {
    if (!this._portMutexes.has(port)) {
      this._portMutexes.set(port, new Mutex());
    }
    return this._portMutexes.get(port)!;
  }

  private async _executeAtomic(port: string, writeBuffer: Buffer, timeoutMs = 5000): Promise<Buffer> {
    const readPromise = this._serialManager.onData(port);
    const timeoutPromise = new Promise<Buffer>((_, reject) => {
      setTimeout(() => {
        reject(new SerialTimeoutError(`Timeout on port ${port} after ${timeoutMs}ms waiting for response.`));
      }, timeoutMs);
    });

    if (writeBuffer.length > 0) {
      await this._serialManager.write(port, writeBuffer);
    }
    return Promise.race([readPromise, timeoutPromise]).then((response) => (Buffer.isBuffer(response) ? response : Buffer.from(response)));
  }
  ...
  private async _handleSingleRequest(request: CuLockRequest, protocol: IProtocol<BaseResponse>): Promise<ScuResponse> {
    ...
    if (!(await this._isPortOpen(port))) {
      throw new Error(`Port not open: ${port}`);
    }

    const mutex = this._getMutexForPort(port);
    const release = await mutex.acquire();
    this._logger.debug(`[${port}] Locked for single request (Device: ${request.deviceId})`);
    ...
    try {
      ...
      if (request.command === Command.OPEN_LOCK) {
        ...
        const statusMsg = protocol.createMessage({
          command: Command.GET_STATUS,
          deviceId: request.deviceId,
          lockId: 0,
        });
        responseBuffer = await this._executeAtomic(port, statusMsg);
      } else {
        responseBuffer = await this._executeAtomic(port, msg);
      }

      return protocol.parseResponse(request.deviceId, request.lockIds, responseBuffer) as ScuResponse;
    } finally {
      this._logger.debug(`[${port}] Unlocked for single request (Device: ${request.deviceId})`);
      release();
    }
  }
  ...
  private async _handleMultipleRequest(request: CuLockRequest, protocol: IProtocol<BaseResponse>): Promise<CuResponse> {
    ...
    const mutex = this._getMutexForPort(port);
    const release = await mutex.acquire();
    this._logger.debug(`[${port}] Locked for multiple request (Device: ${request.deviceId})`);
    ...
    try {
      ...
      if (request.command === Command.OPEN_LOCK) {
        const statusMessages = protocol.createMessages({
          command: Command.GET_STATUS,
          deviceId: request.deviceId,
          lockIds: request.lockIds,
        });
      ...
      const responseBuffer = await this._executeAtomic(port, Buffer.from([]), 5000);

      const result = protocol.parseResponse(request.deviceId, request.lockIds, responseBuffer) as CuResponse;
      return this._filterStatus(request.lockIds, result);
    } finally {
      this._logger.debug(`[${port}] Unlocked for multiple request (Device: ${request.deviceId})`);
      release();
    }
  }
      ...
      ...this._controlUnitLockPorts.map(
        async (cuPort): Promise<SerialPortState> =>
          this._serialManager.open(cuPort, {
            baudRate: 19200,
            bufferStrategy: { type: 'time', timeMs: 500, maxBufferSize: 1024 },
  ...
  private async _scanSinglePort(portPath: string): Promise<void> {
    const mutex = this._getMutexForPort(portPath);
    const release = await mutex.acquire();
    this._logger.log(`[${portPath}] Locked for scanning.`);

    try {
      const state = await this._serialManager.open(portPath, {
        baudRate: 19200,
        bufferStrategy: { type: 'time', timeMs: 1000 },
      });
      ...
      if (await this._isPortOpen(portPath)) {
        await this._serialManager.close(portPath);
      }
      this._logger.log(`[${portPath}] Unlocked after scanning.`);
      release();
    }
  }
  ...
  private async _checkScuHardware(portPath: string): Promise<boolean> {
    try {
      const buffer = await this._executeAtomic(portPath, Buffer.from(DETECT_SCU_PORT_MESSAGE), 2000);
      return this._isScuResponse(buffer);
    } catch (error) {
      if (error instanceof SerialTimeoutError) {
        this._logger.debug(`SCU check timed out on ${portPath} (This is normal if it's not an SCU device)`);
      } else {
        this._logger.error(`SCU check error on ${portPath}:`, { message: error.message });
      }
      return false;
    }
  }
  ...
  private async _checkCuHardware(portPath: string): Promise<boolean> {
    try {
      const buffer = await this._executeAtomic(portPath, Buffer.from(DETECT_CU_PORT_MESSAGE), 2000);
      return this._isCuResponse(buffer);
    } catch (error) {
      if (error instanceof SerialTimeoutError) {
        this._logger.debug(`CU check timed out on ${portPath} (This is normal if it's not a CU device)`);
      } else {
        this._logger.error(`CU check error on ${portPath}:`, { message: error.message });
      }
      return false;
    }
  }
  ...
  private _filterStatus(lockIds: number[], result: CuResponse): CuResponse {
    ...
    const status: { [lockId: number]: LOCK_STATUS } = {};
    for (const lockId of lockIds) {
      if (result.lockStatuses && result.lockStatuses[lockId] !== undefined) {
