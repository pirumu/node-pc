...
export class SnowflakeId {
  ...
  public readonly DEFAULT_EPOCH_DATETIME = new Date('2024-01-01T00:00:00Z').getTime();
  ...
  public readonly MAX_DATA_CENTER_ID: number = (1 << this.DATA_CENTER_ID_BITS) - 1;
  ...
  constructor(opts?: SnowflakeIdOptions) {
    const { workerId: workerIdOpt, dataCenterId: dataCenterIdOpt } = opts?.machineId || {};

    this._workerId = workerIdOpt ?? random(Number(this.MAX_WORKER_ID));

    if (this._workerId > this.MAX_WORKER_ID) {
      throw new RangeError(`Worker ID must be between 0 and ${this.MAX_WORKER_ID}`);
    }

    this._dataCenterId = dataCenterIdOpt ?? random(Number(this.MAX_DATA_CENTER_ID));

    if (this._dataCenterId > this.MAX_DATA_CENTER_ID) {
      throw new RangeError(`Data Center ID must be between 0 and ${this.MAX_DATA_CENTER_ID}`);
    }
  ...
  public generate(): bigint {
    const currentTimestamp = currentTime(this._customEpoch);

    this._sequence = this.makeSequence(currentTimestamp);

    const timestampBits = currentTimestamp.toString(2).padStart(this.EPOCH_BITS, '0');

    const dataCenterBits = this._dataCenterId.toString(2).padStart(this.DATA_CENTER_ID_BITS, '0');

    const workerIdBits = this._workerId.toString(2).padStart(this.WORKER_ID_BITS, '0');

    const sequenceBits = this._sequence.toString(2).padStart(this.SEQUENCE_BITS, '0');

    const id = BigInt(`0b${timestampBits}${dataCenterBits}${workerIdBits}${sequenceBits}`);
  ...
  public decode(id: bigint): SnowflakeIdDecoded {
    ...
    const timestampLeftShift = this.DATA_CENTER_ID_BITS + this.WORKER_ID_BITS + this.SEQUENCE_BITS;

    const binaryTimestamp = idBinary.substring(0, idBinary.length - timestampLeftShift);
    const binarySequence = idBinary.substring(idBinary.length - this.SEQUENCE_BITS);
    ...
    return {
      ...
      dateTime: new Date(Number(timestamp) + this._customEpoch),
    };
  }

  protected makeSequence(currentTimestamp: number) {
    ...
    this._lastTimestamp = waitUntilNextMillisecond(this._customEpoch, currentTimestamp);
    this._sequence = 0;
    return this._sequence;
  }
}
...
function waitUntilNextMillisecond(customEpoch: number, currentTimestamp: number) {
  let nextTimestamp = currentTime(customEpoch);

  while (nextTimestamp <= currentTimestamp) {
    nextTimestamp = currentTime(customEpoch);
  }
